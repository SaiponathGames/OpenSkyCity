def user_writer(x, h, k, j, p):
    ...
    for i in range(p2):
        ...
        id1 = ran.randint(k, j)
        p3.append(str(id1))
        yt = ''.join(l + '-' * (n % 5 == 4) for n, l in
                     enumerate(ran.choice(stri.ascii_uppercase + stri.digits) for _ in range(p))).rstrip('-')
        p5.append(yt)
        p6 = ''.join(
                    ...
                    if x == p4[k]:
                        if us1 in p2:
                            if p2.index(us1) == p4.index(x):
                                z = p2[int(k)]
                                return "The key " + p.bold + x + p.normal + " is a valid " + n + " version key. The username of key " + \
            ...
            for u87 in p1:
                ...
                if ui:
                    ...
                    if p99 in p1:
                        ...
                        return "Can't able to find the userID " + io + \
